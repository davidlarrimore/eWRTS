...
export default class RAILSRecordPageWidget extends LightningElement {
  ...
  getRecord() {
    getRailsRecordByCase({ recordId: this.recordId })
      .then((data) => {
        console.log(
          `RAILSRecordPageWidget.getRailsRecordByCase Completed Successfully: ${JSON.stringify(
            data
          )}`
        );
        ...
        if (
          newItem.caseRecord.File_Pulled_from_Shelf__c === true &&
          newItem.caseRecord.File_Delivered__c === false
        ) {
          ...
          newItem.railsRecord.date_received_in_rpc =
            newItem.caseRecord.Date_File_Pulled_from_Shelf__c;
        ...
        if (
          newItem.caseRecord.File_Pulled_from_Shelf__c === true &&
          newItem.caseRecord.File_Delivered__c === true
        ) {
          ...
          newItem.railsRecord.date_received_in_rpc =
            newItem.caseRecord.Date_File_Delivered__c;
          newItem.railsRecord.age_with_custodian = 0;
        }

        //eWRTS File Pulled for Processing
        if (
          newItem.caseRecord.File_Pulled_for_Processing__c === true &&
          newItem.caseRecord.File_Processed__c === false
        ) {
      ...
      .catch((error) => {
        ...
        this.apiCallCompletedFlag = true;
        console.log(
          `RAILSRecordPageWidget.getRailsRecordByCase Failed: ${error}`
        );
      });
  }
